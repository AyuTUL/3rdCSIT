...
int main()
{
	...
	cout<<"Enter scaling factor(sx,xy): ";cin>>sx>>sy;
	cout<<"Enter rotation angle(in degrees): ";cin>>angle;
	int cx = (x1+x2+x3)/3;
	int cy = (y1+y2+y3)/3;
	drawTriangle(x1,y1,x2,y2,x3,y3,WHITE,"Original Triangle");
	...
	drawTriangle(x1_scaled,y1_scaled,x2_scaled,y2_scaled,x3_scaled,y3_scaled,RED,"Centroid Scaled");
	...
	drawTriangle(x1_rot,y1_rot,x2_scaled,y2_rot,x3_rot,y3_rot,BLUE,"Rotated");
	getch();
	closegraph();
	return 0;
}