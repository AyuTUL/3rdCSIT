...
int restoringDivision(int dividend, int divisor, int &remainder_out, int qbit) {
    if (divisor == 0) {
    ...
    int abit = qbit + 1; // A and M are 1 bit wider
    ...
    cout << "\nRestoring Division Tracing Table:\n";
    cout << HR;
    ...
    for (int i = qbit - 1; i >= 0; i--) {
        ...
        if (A & (1 << (abit - 1))) {
            Q &= ~1;
            A = (A + M) & ((1 << abit) - 1);
            row(i, A, Q, abit, qbit, "Q[0] <- 0 & Restore");
        } else {
            Q |= 1;
            row(i, A, Q, abit, qbit, "Q[0] <- 1");
        }
    }
...
int main() {
    ...
    if (dividend < 0 || divisor < 0) qbit++;
    if (qbit > 8) {
        cout << "Bit size exceeds 8. Capping to 8 bits.\n";
        qbit = 8;
    }

    int quotient = restoringDivision(dividend, divisor, remainder, qbit);
